...
abstract class Db implements DbInterface
{
    ...
    public function optInit()
    {
        ...
        $opt = array(
            ...
            'fieldHaveSet' => false, //字段值是否已经设置，field()方法传第2个参数为true时，此为真，用于count
            'where' => '',
            'like' => '',
            'group' => '',
            'having' => '',
    ...
    private function statistics($type, $opt)
    {
        ...
        //字段没有设置值时，设置字段为count()等的字段值
        if (!$this->opt['fieldHaveSet']) {
...
AND ";
    ...
    public function field($opt)
    {
        ...
        if (isset($opt[1]) && $opt[1] === true) {
            $this->opt['field'] = $opt[0];
            $this->opt['fieldHaveSet'] = true;
            return;
        }
        $field = array();
        foreach ($opt as $v) {
